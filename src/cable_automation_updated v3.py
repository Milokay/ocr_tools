...
class DynamicPriceUpdater:
        ...
        try:
            ...
            for weight in expected_weights:
                ...
                if weight_str in text:
                    ...
                    # Try fuzzy matching with wider range
                    weight_int = int(weight)
                    for i in range(-10, 11):  # Check +/- 10 for OCR errors
    ...
    def find_weight_coordinates(self, target_weights: List[float]) -> Dict[float, Tuple[int, int]]:
        ...
        try:
            ...
            for i in range(n_boxes):
                ...
                if text and confidence > 20:  # Even lower threshold for weight detection
            ...
            for target_weight in target_weights:
                ...
                for element in text_elements:
                    ...
                    # Try fuzzy matching for slight OCR variations - increased range
                    for element in text_elements:
                        # Check if OCR text is close to target weight
                        try:
                            ocr_number = int(element['text'])
                            if abs(ocr_number - target_weight) <= 10:  # Increased tolerance to 10
                                weight_x = element['center_x']
                                weight_y = element['center_y']
                                price_x = 1196
                                price_y = weight_y
            ...
            # If some weights are still missing, try position-based search
            if len(weight_coordinates) < len(target_weights):
                self.logger.info(f"🔍 Some weights missing ({len(weight_coordinates)}/{len(target_weights)}), trying position-based search...")
                additional_coords = self.find_missing_weight_by_position(target_weights, weight_coordinates)
                weight_coordinates.update(additional_coords)
                
                if additional_coords:
                    self.logger.info(f"✅ Position-based search found {len(additional_coords)} additional weights")
            
            if not weight_coordinates:
                self.logger.warning("⚠️ No weight coordinates found. Running comprehensive scan...")
                
                # Comprehensive scan for missing weights
                self.logger.info("🔍 COMPREHENSIVE SCAN - All detected text with confidence > 5:")
                comprehensive_scan = pytesseract.image_to_data(gray, output_type=pytesseract.Output.DICT)
                
                scan_results = []
                n_boxes = len(comprehensive_scan['text'])
                for i in range(n_boxes):
                    text = comprehensive_scan['text'][i].strip()
                    confidence = int(comprehensive_scan['conf'][i])
                    
                    if text and confidence > 2:  # Very low threshold
                        x = comprehensive_scan['left'][i] + comprehensive_scan['width'][i] // 2
                        y = comprehensive_scan['top'][i] + comprehensive_scan['height'][i] // 2
                        scan_results.append((text, x, y, confidence))
                
                # Show all 4-5 digit numbers
                potential_weights = []
                for text, x, y, conf in scan_results:
                    if text.isdigit() and 4 <= len(text) <= 5:
                        potential_weights.append((text, x, y, conf))
                
                self.logger.info(f"📊 ALL 4-5 digit numbers found (confidence > 5):")
                for i, (text, x, y, conf) in enumerate(potential_weights):
                    is_target = any(text == str(int(tw)) for tw in target_weights)
                    marker = "🎯 TARGET" if is_target else "  "
                    self.logger.info(f"  {i+1:2d}. '{text}' at ({x:4d}, {y:3d}) conf:{conf:2d} {marker}")
    ...
    def find_missing_weight_by_position(self, target_weights: List[float], found_coordinates: Dict[float, Tuple[int, int]]) -> Dict[float, Tuple[int, int]]:
        """Try to find missing weights by analyzing position patterns"""
        try:
            missing_weights = [w for w in target_weights if w not in found_coordinates]
            if not missing_weights:
                return {}
            
            self.logger.info(f"🔍 Searching for missing weights by position analysis: {missing_weights}")
            
            # Analyze Y-coordinate pattern from found weights
            if len(found_coordinates) >= 2:
                y_coords = [coords[1] for coords in found_coordinates.values()]
                y_coords.sort()
                
                if len(y_coords) >= 2:
                    y_spacing = y_coords[1] - y_coords[0]  # Typical row spacing
                    self.logger.info(f"📏 Detected row spacing: {y_spacing} pixels")
                    
                    # Predict where missing weight should be
                    min_y = min(y_coords)
                    max_y = max(y_coords)
                    
                    # Check positions above, between, and below
                    candidate_positions = []
                    candidate_positions.append(min_y - y_spacing)  # Above first
                    candidate_positions.append(max_y + y_spacing)  # Below last
                    
                    # Between existing weights
                    for i in range(len(y_coords) - 1):
                        between_y = (y_coords[i] + y_coords[i + 1]) // 2
                        candidate_positions.append(between_y)
                    
                    self.logger.info(f"🎯 Checking candidate Y positions: {candidate_positions}")
                    
                    # Take fresh screenshot for position-based search
                    screenshot = pyautogui.screenshot()
                    screenshot_np = np.array(screenshot)
                    screenshot_bgr = cv2.cvtColor(screenshot_np, cv2.COLOR_RGB2BGR)
                    gray = cv2.cvtColor(screenshot_bgr, cv2.COLOR_BGR2GRAY)
                    
                    # Get all text with very low confidence
                    ocr_data = pytesseract.image_to_data(gray, output_type=pytesseract.Output.DICT)
                    
                    additional_coordinates = {}
                    
                    for missing_weight in missing_weights:
                        missing_str = str(int(missing_weight))
                        
                        # Search near candidate positions
                        for candidate_y in candidate_positions:
                            # Look for text near this Y position
                            n_boxes = len(ocr_data['text'])
                            for i in range(n_boxes):
                                text = ocr_data['text'][i].strip()
                                confidence = int(ocr_data['conf'][i])
                                
                                if confidence > 2:  # Very low threshold
                                    x = ocr_data['left'][i] + ocr_data['width'][i] // 2
                                    y = ocr_data['top'][i] + ocr_data['height'][i] // 2
                                    
                                    # Check if this text is near the candidate Y position
                                    if abs(y - candidate_y) <= 25:  # Within 25 pixels
                                        # Check if it matches our missing weight (exact or close)
                                        if text == missing_str:
                                            price_x = 1196
                                            price_y = y
                                            additional_coordinates[missing_weight] = (price_x, price_y)
                                            self.logger.info(f"✅ Found missing weight {missing_weight} by position at ({x}, {y}) → Price at ({price_x}, {price_y})")
                                            break
                                        elif text.isdigit() and abs(int(text) - missing_weight) <= 10:
                                            price_x = 1196
                                            price_y = y
                                            additional_coordinates[missing_weight] = (price_x, price_y)
                                            self.logger.info(f"✅ Found missing weight {missing_weight} as '{text}' by position at ({x}, {y}) → Price at ({price_x}, {price_y})")
                                            break
            ...
            self.logger.error(f"❌ Position-based search failed: {e}")
            return {}
    ...
    def debug_weight_detection(self, target_weights: List[float]) -> Dict:
        ...
        try:
            ...
            for i in range(n_boxes):
                ...
                if text and confidence > 20:  # Much lower threshold for debugging
                    x = ocr_data['left'][i] + ocr_data['width'][i] // 2
                    y = ocr_data['top'][i] + ocr_data['height'][i] // 2
            ...
            # Show first 20 numbers found for better debugging
            self.logger.info(f"📋 Numbers detected (first 20):")
            for i, num in enumerate(all_numbers[:20]):
                self.logger.info(f"  {i+1}. '{num['text']}' at ({num['x']}, {num['y']}) conf:{num['confidence']}")
            
            # Specifically look for the missing weight with fuzzy matching
            self.logger.info(f"\n🔍 SPECIFIC SEARCH for missing weights:")
            for target_weight in target_weights:
                found = False
                ...
                if not found:
                    self.logger.info(f"❌ Missing weight: {target_weight}")
                    # Look for close matches
                    close_matches = []
                    for num in all_numbers:
                        try:
                            num_val = int(num['text'])
                            if abs(num_val - target_weight) <= 10:  # Within 10 of target
                                close_matches.append(num)
                        except:
                            continue
                    
                    if close_matches:
                        self.logger.info(f"  🔍 Close matches for {target_weight}:")
                        for match in close_matches:
                            self.logger.info(f"    '{match['text']}' at ({match['x']}, {match['y']}) conf:{match['confidence']}")
                    else:
                        self.logger.info(f"  ❌ No close matches found for {target_weight}")
